...
public interface FormCreateMapper extends BaseMapper<FormCreate> {
    @Select("select * from form_create where company = #{company}")
    List<FormCreate> getList(String company);

    @Select("select * from form_create where company = #{company} and form_name like '%' + #{query} + '%'")
    List<FormCreate> queryList(String company,String query);

    @Select("select * from form_create where company = #{company} and create_name = #{create}")
    List<FormCreate> getListByName(String company,String create);

    @Select("select * from form_create where company = #{company} and create_name = #{create} and form_name like '%' + #{query} + '%'")
